...
test('--fix and --stdin shows help', async t => {
  const result =
      await execa('../markdownlint.js', [ '--fix', '--stdin', 'correct.md' ],
                  {stripFinalNewline : false});
  t.true(result.stdout.includes('--help'));
  t.is(result.stderr, '');
});
...
test('linting of correct Markdown file yields no output with absolute path',
     async t => {
...
test('linting of incorrect Markdown via npm run file fails with eol',
     async t => {
       try {
...
test('dir linting works with failing .markdown files and absolute path',
     async t => {
       try {
         await execa(
             '../markdownlint.js',
...
test('glob linting with failing files passes when failures ignored by glob',
     async t => {
       const result = await execa('../markdownlint.js',
                                  [
                                    '--config', 'test-config.json', '**/*.md',
...
test('glob linting with failing files passes when everything ignored by glob',
     async t => {
       const result = await execa(
           '../markdownlint.js',
...
test('glob linting with failing files has fewer errors when ignored by dir',
     async t => {
       try {
         await execa('../markdownlint.js',
                     [
...
test('dir linting with failing files has fewer errors when ignored by file',
     async t => {
       try {
         await execa('../markdownlint.js',
                     [
...
test('glob linting with failing files passes when ignored by multiple globs',
     async t => {
       const result =
           await execa('../markdownlint.js',
                       [
...
test('--output with invalid path fails', async t => {
  ...
  try {
    ...
    t.is(error.stdout, '');
    t.is(error.stderr.replace(/: ENOENT[^]*$/, ''),
...
test('.markdownlint.json in cwd is used automatically',
     getCwdConfigFileTest('json'));

test('.markdownlint.yaml in cwd is used automatically',
     getCwdConfigFileTest('yaml'));

test('.markdownlint.yml in cwd is used automatically',
...
test('.markdownlint.yaml in cwd is used instead of .markdownlint.yml',
     getCwdConfigFileTest('yaml-yml'));

test('.markdownlint.json with JavaScript-style comments is handled',
...
test('Custom rule from node_modules package loaded relative to cwd',
     async t => {
       try {
         const input = '# Input\n';
         await execa(
